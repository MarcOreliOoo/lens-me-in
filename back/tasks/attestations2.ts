...
task('attestation2', 'tests the attestation2 module').setAction(async ({ }, hre) => {
    ...
    console.log(`user balance of: ${contract.balanceOf(user.address, 1)}`);
    ...
    const data_comment_user1 = defaultAbiCoder.encode(
        ['address'],
        [user.address]
    )
    ...
    const data_comment_user2 = defaultAbiCoder.encode(
        ['address'],
        [user2.address]
    )
